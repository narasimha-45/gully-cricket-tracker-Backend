...
const extractIds = (match) => {
  ...
  if (match.result?.manOfTheMatchId)
    addPlayer(match.result.manOfTheMatchId);
  ...
  (match.innings || []).forEach((inn) => {
    ...
    Object.values(inn.battingStats || {}).forEach((stat) => {
      ...
      if (stat.dismissal?.bowlerId)
        addPlayer(stat.dismissal.bowlerId);

      if (stat.dismissal?.fielderId)
        addPlayer(stat.dismissal.fielderId);
    });
    ...
    Object.values(inn.dismissals || {}).forEach((dismissal) => {
      ...
      if (dismissal.bowlerId)
        addPlayer(dismissal.bowlerId);

      if (dismissal.fielderId)
        addPlayer(dismissal.fielderId);
    });
    ...
    (inn.ballByBall || []).forEach((ball) => {
      ...
      if (ball.wicket) {
        ...
        if (ball.wicket.helperId)
          addPlayer(ball.wicket.helperId);
      }
    });
  });
...
export const denormalizeMatch = async (match) => {
  ...
  const getTeamName = (id) =>
    id
      ? teamIdToName[id.toString()] || id.toString()
      : null;

  const getPlayerName = (id) =>
    id
      ? playerIdToName[id.toString()] || id.toString()
      : null;
  ...
  const denormalizedMatches = matches.map((m) => {
    ...
    if (denorm.toss?.winnerTeamId) {
      denorm.toss = {
        ...denorm.toss,
        winner: getTeamName(denorm.toss.winnerTeamId),
      };
    ...
    if (denorm.result?.winnerTeamId) {
      denorm.result = {
        ...denorm.result,

        winner: getTeamName(
          denorm.result.winnerTeamId
        ),

        manOfTheMatch: getPlayerName(
          denorm.result.manOfTheMatchId
        ),
      };
    ...
    if (denorm.innings) {
      denorm.innings = denorm.innings.map((inn) => {
        ...
        dInn.battingTeam = getTeamName(
          dInn.battingTeamId
        );

        dInn.bowlingTeam = getTeamName(
          dInn.bowlingTeamId
        );
        ...
        if (dInn.battingStats) {
          ...
          Object.entries(dInn.battingStats).forEach(
            ([playerKey, stat]) => {
              const pName = stat.playerId
              ...
              if (dStat.dismissal) {
                ...
                if (dDismissal.bowlerId) {
                  dDismissal.bowler = getPlayerName(
                    dDismissal.bowlerId
                  );
                ...
                if (dDismissal.fielderId) {
                  dDismissal.fielder = getPlayerName(
                    dDismissal.fielderId
                  );
        ...
        if (dInn.bowlingStats) {
          ...
          Object.entries(dInn.bowlingStats).forEach(
            ([playerKey, stat]) => {
              const pName = stat.playerId
                ? getPlayerName(stat.playerId)
                : playerKey;
        ...
        if (dInn.dismissals) {
          ...
          Object.entries(dInn.dismissals).forEach(
            ([playerKey, dismissal]) => {
              const pName = dismissal.playerId
                ? getPlayerName(dismissal.playerId)
                : playerKey;
        ...
        if (dInn.ballByBall) {
          dInn.ballByBall = dInn.ballByBall.map(
            (ball) => {
              const dBall = { ...ball };

              dBall.striker = getPlayerName(
                dBall.strikerId
              );

              dBall.nonStriker = getPlayerName(
                dBall.nonStrikerId
              );

              dBall.bowler = getPlayerName(
                dBall.bowlerId
              );
              ...
              if (dBall.wicket) {
                ...
                dBall.wicket.outBatsman =
                  getPlayerName(
                    dBall.wicket.outBatsmanId
                  );
        ...
        if (dInn.battingOrder) {
          dInn.battingOrder =
            dInn.battingOrder.map(getPlayerName);
        }
