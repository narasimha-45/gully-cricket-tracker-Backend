...
import PlayerProfile from "../../../models/PlayerProfile.js"; // ← add
import TeamProfile from "../../../models/TeamProfile.js"; // ← add
...
const buildBulkOps = ({ documents, filterBuilder, label }) => {
  if (!documents || !(documents instanceof Map)) return [];
...
/* ======================================================
   BUILD PROFILE SYNC OPS
====================================================== */

const buildPlayerProfileSyncOps = (overallPlayerStats) => {
  if (!overallPlayerStats || !(overallPlayerStats instanceof Map)) return [];

  const operations = [];

  for (const document of overallPlayerStats.values()) {
    if (!document?.playerId) continue;

    operations.push({
      updateOne: {
        filter: { _id: document.playerId },
        update: {
          $set: {
            totalMatches: document.totalMatches || 0,
            seasonsPlayed: document.seasonsPlayed || [],
            teamsPlayedFor: document.teamsPlayedFor || [],
          },
        },
      },
    });
...
const buildTeamProfileSyncOps = (overallTeamStats) => {
  if (!overallTeamStats || !(overallTeamStats instanceof Map)) return [];

  const operations = [];

  for (const document of overallTeamStats.values()) {
    if (!document?.teamId) continue;

    operations.push({
      updateOne: {
        filter: { _id: document.teamId },
        update: {
          $set: {
            totalMatches: document.stats?.played || 0,
            seasonsPlayed: document.seasonsPlayed || [],
          },
        },
      },
...
export const flushAccumulators = async (accumulators) => {
  /* =========================================
     BUILD OPERATIONS
  ========================================= */
  for (const doc of accumulators.overallTeamStats.values()) {
    if (doc.seasonsPlayed instanceof Set) {
      doc.seasonsPlayed = [...doc.seasonsPlayed];
    }
  }

  for (const doc of accumulators.overallPlayerStats.values()) {
    if (doc.seasonsPlayed instanceof Set)
      doc.seasonsPlayed = [...doc.seasonsPlayed];
    if (doc.teamsPlayedFor instanceof Set)
      doc.teamsPlayedFor = [...doc.teamsPlayedFor];
  }

  const overallPlayerStatsOps = buildBulkOps({
    documents: accumulators.overallPlayerStats,
    label: "OVERALL_PLAYER_STATS",
    filterBuilder: (document) => ({ playerId: document.playerId }),
  });
  ...
  const overallTeamStatsOps = buildBulkOps({
    ...
    filterBuilder: (document) => ({ teamId: document.teamId }),
  });
  ...
  /* ── Profile sync ops ── */
  const playerProfileSyncOps = buildPlayerProfileSyncOps(
    accumulators.overallPlayerStats,
  );

  const teamProfileSyncOps = buildTeamProfileSyncOps(
  ...
  await Promise.all([
    ...
    safeBulkWrite({
      model: PlayerProfile,
      operations: playerProfileSyncOps,
      label: "PLAYER_PROFILE_SYNC",
    }),

    safeBulkWrite({
      model: TeamProfile,
      operations: teamProfileSyncOps,
      label: "TEAM_PROFILE_SYNC",
    }),
